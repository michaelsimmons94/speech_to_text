...
"""
speechToText.py is a wrapper for pocketsphinx.
  parameters:
...
class speechToText(object):
    """ GStreamer based speech speechToText. """
    ...
    def __init__(self):
        ...
        if rospy.has_param(self._lm_param) and rospy.has_param(self._dic_param):
            self.start_speechToText()
        else:
            rospy.logwarn("lm and dic parameters need to be set to start speechToText.")

    def start_speechToText(self):
        # rospy.loginfo("Starting speechToText... ")
        ...
        self.asr.set_property('logLevel',"OFF")
        # Configure language model
        if rospy.has_param(self._lm_param):
            lm = rospy.get_param(self._lm_param)
        else:
            rospy.logerr('speechToText not started. Please specify a language model file.')
            return

        if rospy.has_param(self._dic_param):
            dic = rospy.get_param(self._dic_param)
        else:
            rospy.logerr('speechToText not started. Please specify a dictionary.')
            return
    ...
    def start(self, req):
        self.start_speechToText()
        rospy.loginfo("speechToText started")
        return EmptyResponse()

    def stop(self, req):
        self.stop_speechToText()
        rospy.loginfo("speechToText stopped")
        return EmptyResponse()
